...
function renderTodos() {
  ...
  spisok.forEach((item, index) => {
    ...
    const buttonModal = $("<button>").addClass("modalStyle").attr("type", "button").attr("data-bs-toggle", "modal").attr("data-bs-target", "#" + modalId);
    ...
    const modal = $("<div>").addClass("modal fade").attr("id", modalId).attr("tabindex", "-1").attr("aria-labelledby", "exampleModalLabel").attr("aria-hidden", "true");
    const modalDialog = $("<div>").addClass("modal-dialog");
    const modalContent = $("<div>").addClass("modal-content");
    const modalHeader = $("<div>").addClass("modal-header");
    const modalH1 = $("<h1>").addClass("modal-title fs-5").attr("id", "exampleModalLabel").text("some");
    const modelButton = $("<button>").attr("type", "button").addClass("btn-close").attr("data-bs-dismiss", "modal").attr("aria-labelaria-label", "Close");
    const modalBody = $("<div>").addClass("modal-body").text("...");
    const modalFooter = $("<div>").addClass("modal-footer");
    const buttonClose = $("<button>").addClass("btn btn-secondary").attr("data-bs-dismiss", "modal").text("Close");
